#!/bin/bash
#
# Sets up a composite HID + ECM + Mass Storage USB device.
#
# Expects to be run on the Debian Jessie base image
# provided by Inverse Path.
#
# Usage:
#   # ./hid-ecm-ms.sh <path-to-img>
#
# <path-to-img> must be the full path to a disk image file.
#
# The easiest way to make this disk image file is to create
# a blank file of the desired size, set up the USBArmory as
# a mass storage device (e.g. with this script) and then use
# the host to format and partition the mass storage device
# exposed by the USBArmory as desired.
# Example execution:
#
#   $ dd if=/dev/zero of=disk.img bs=1M count=1024
#   # ./hid-ecm-ms.sh /home/usbarmory/disk.img
#
# Then on the host computer you'll see a new block device,
# e.g. /dev/sdb, and from there you can use fdisk or gparted
# or what have you to create a partition table, partitions,
# and a filesystem.
#
...
# insert modules for HID, ECM Ethernet, and Mass Storage
modprobe usb_f_hid
modprobe usb_f_ecm
...
# enable hid, ecm and mass storage functions
...
# configure mass storage function by setting path to image to expose as the disk device
echo $1 > functions/mass_storage.ms0/lun.0/file
